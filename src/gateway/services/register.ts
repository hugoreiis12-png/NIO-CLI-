...
export type RegisterOutcome =
  ...
  | { ok: false; reason: 'invalid_name' | 'weak_password' | 'breached_password' | 'name_taken' };
...
export async function register(
  ...
  // SP-7: roda pra todo mundo antes do check de nome — não vaza enumeração (é
  // sobre a senha, não o nome).
  if ((await checkPasswordBreach(password)).breached) return { ok: false, reason: 'breached_password' };
